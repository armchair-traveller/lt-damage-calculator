...
import { isValidLiveBuildSlug, pinLiveBuild } from '$lib/server/live-builds/service';
...
export const POST: RequestHandler = async ({ request, params }) => {
	try {
		...
		return json(
			{ build: await pinLiveBuild(params.slug, actorId, body) },
			{ headers: { 'cache-control': 'no-store' } }
		);
	} catch (error) {
		return liveBuildErrorResponse(error, 'pin-live-build');
	}
};
